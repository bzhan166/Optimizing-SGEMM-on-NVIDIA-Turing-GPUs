...
void mysgemm_v6(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    int row1 = (tx&7)<<2, row2 = row1+1, row3 = row1+2, row4 = row1+3, col = tx>>3;//8 = 32/4, com = row(tx&_)
    A = &A((bx<<5),0);
    B = &B(0,(by<<5));
    C = &C((bx<<5),(by<<5));
...
#define B(i,j) B[(i) + (j)*ldb]
#define C(i,j) C[(i) + (j)*ldc]
#define sa4(i, j) sa4[(i) + ((j)<<5)]
#define sb4(i, j) sb4[(i) + ((j)<<5)]
#define MS 32
#define NS 32
#define KS 32
...
void mysgemm_v6(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    float4 Av, Bv, Cv, Cres;//replace tmp
    Cres.x = 0., Cres.y = 0., Cres.z = 0., Cres.w = 0.;  
    float b00;
    for (int k_count = 0; k_count<K; k_count+=KS){
        Av = *((float4 *)(&A(row1, col)));
        ...
        for(int inner_k_count=0; inner_k_count<KS; inner_k_count++){
            ...
            Cres.x += sa4(row1, inner_k_count) * b00;
            Cres.y += sa4(row2, inner_k_count) * b00;
            Cres.z += sa4(row3, inner_k_count) * b00;
            Cres.w += sa4(row4, inner_k_count) * b00;
        }
        __syncthreads();
    }
    Cv = *((float4 *)(&C(row1, col)));
