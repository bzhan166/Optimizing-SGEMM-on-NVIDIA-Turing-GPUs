...
void mysgemm_v8(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    int row_a = (tx&31)<<2, col_a = tx>>5;//128/4 = 32 ; >>5 =32
    int row_b = (tx&1)<<2, col_b = tx>>1;//8/4=2
    int lda8 = lda<<3; //ks=8
    int row_c = (tx&15)<<3, col_c = (tx>>4)<<3;//(Mr,Nr=(8,8),128/8=16)
    A = &A((bx<<7),0);
    B = &B(0,(by<<7));
...
#define B(i,j) B[(i) + (j)*ldb]
#define C(i,j) C[(i) + (j)*ldc]
#define sa8(i, j) sa8[(i) + ((j)<<7)]
#define sb8(i, j) sb8[(i) + ((j)<<7)]
#define MS_8 128
#define NS_8 128
#define KS_8 8
//v1 += v2 * s3, vector scaling
...
void mysgemm_v8(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    __shared__ float sa8[1024];
    __shared__ float sb8[1024];
    float4 Av1, Av2, Bv1, Bv2, Cv[16], Cres[16];//2*4*2
    memset(Cres, 0, sizeof(Cres));
    for (int k_count = 0; k_count<K; k_count+=KS_8){
        vload(Av1, &A(row_a,col_a))
        vload(Bv1, &B(row_b,col_b))
        ((float4 *)sa8)[tx] = Av1;
        sa8(col_b,row_b)=Bv1.x;
        sa8(col_b,row_b+1)=Bv1.y;
        sa8(col_b,row_b+2)=Bv1.z;
        sa8(col_b,row_b+3)=Bv1.w;
        A+=lda8;B+=8;
        __syncthreads();
        #pragma unroll
        for(int inner_k_count=0; inner_k_count<KS_8; inner_k_count++){
