...
void mysgemm_v7(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    int row_a = (tx&15)<<2, col_a = tx>>4;//16 = 64/4 col = tx/16
    int row_b = (tx&3)<<2, col_b = tx>>2;//4 = 16/4 col = 4
    int col_c = col_a<<2; //tx/4(Mr,Nr = 4,4) cpl_c = col_a*4 = tx/16*4=tx/4
    int lda16 = lda<<4; //ks=16
    A = &A((bx<<6),0);
    B = &B(0,(by<<6));
...
#define B(i,j) B[(i) + (j)*ldb]
#define C(i,j) C[(i) + (j)*ldc]
#define sa7(i, j) sa7[(i) + ((j)<<6)]
#define sb7(i, j) sb7[(i) + ((j)<<6)]
#define MS_7 64
#define NS_7 64
#define KS_7 16
//v1 += v2 * s3, vector scaling
...
void mysgemm_v7(int M, int N, int K, float alpha, float* A, float* B, float beta, float* C){
    ...
    int tx = threadIdx.x;
    int bx = blockIdx.x, by = blockIdx.y;
    int row_a = (tx&15)<<2, col_a = tx>>4;//M K
    int row_b = (tx&3)<<2, col_b = tx>>2;//K N
    int col_c = col_b;//tx/16*4=tx/4 N
    int lda16 = lda<<4;
    A = &A((bx<<6),0);
    B = &B(0,(by<<6));
    C = &C((bx<<6),(by<<6));
    __shared__ float sa7[MS_7*KS_7];
    __shared__ float sb7[KS_7*NS_7];
    float4 Av, Bv, Cv[4], Cres[4];//register
    memset(Cres, 0, sizeof(Cres));
    for (int k_count = 0; k_count<K; k_count+=KS_7){
        vload(Av, &A(row_a, col_a))
        vload(Bv, &B(row_b, col_b))
        ...
        for(int inner_k_count=0; inner_k_count<KS_7; inner_k_count++){
            vload(Av, &sa7(row_a,inner_k_count))
            vload(Bv, &sb7(col_b,inner_k_count))
            vscal(Cres[0], Av, Bv.x)
            vscal(Cres[1], Av, Bv.y)
            vscal(Cres[2], Av, Bv.z)
            vscal(Cres[3], Av, Bv.w)
